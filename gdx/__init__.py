...
class File:
    ...
    def __init__(self, filename='', mode='r', match_domains=True, lazy=False):
        ...
        # reference them as domain dimensions. In order to make multiple passes,
        # use a queue and reinsert objects which need to be rechecked
    ...
    def sets(self):
        ...
        return filter(lambda s: isinstance(s, Set) and s.name != '*', values)
    ...
    def parameters(self):
        ...
        if sys.version_info[0] >= 3:
            ...
            values = self._symbols.itervalues()
        return filter(lambda s: isinstance(s, Parameter), values)
    ...
    def _match_domain(self, symbol):
        ...
        domain = [None for i in range(symbol.dim)]
        ...
        if len(s) == len(domain_index):
            ...
            for i in range(symbol.dim):
                # if there is no content along this dimension, there is no basis
                # for guessing. Use the root set by default
                if len(s[i]) == 0:
...
class Symbol:
    ...
    def _load(self):
        ...
        data through the API. Currently File.__init__ forcibly loads all defined
        symbols, but a clever hacker could implement lazy-loading, whereby data
        or even symbol information are only loaded upon access.
        """
    ...
    def domain_sets(self):
        ...
        Symbol.dim. Each list element is a Python set containing every value for
    ...
    def create(type_code, gdxfile, index):
        ...
        The created symbol is of one of the types named in type_str. *type_code*
        is one of gdxcc.GMS_DT_EQU, gdxcc.GMS_DT_PAR,, gdxcc.GMS_DT_SET or
        gdxcc.GMS_DT_VAR. Symbol information and data are loaded from the symbol
        at position *index* of the File instance *gdxfile*.
        """
...
class Set(Symbol):
    ...
    def depth(self):
        ...
        …the depth of set ``a`` is 1, the depth of set ``b`` is 2, and the depth
        of set ``c`` is 3.

        To assist :meth:`File._match_domain`, a large value is returned when the
        Set's domain is not available.
        """
        if self.name == '*':
            return 0
...
class Parameter(Symbol):
    ...
    The values are accessible via indexing. For example, if 'myparam' is defined
    by the following GAMS code::
    ...
    subset of data; it will simply fail. Integer indexing is also not supported.
    """
    def _load(self):
        """Implementation of Symbol._load()."""
        # don't load twice
        ...
        for i in range(records):
            ...
            self.value[indices] = value[gdxcc.GMS_VAL_LEVEL]
        if self.dim == 0:
            self.domain = []
        self._loaded = True
    ...
    def __getitem__(self, key):
        ...
        """
        if any([type(k) == slice for k in key]):
            indices = []
            for i, k in enumerate(key):
                if type(k) == slice:
                    ...
                    indices.append((k,))
            return [self.value.get(k, None) for k in product(*indices)]
    ...
    def _valid_key(self, key):
        ...
        if self.dim == 0:
            return False
        return all([(key[i] in self.domain[i].elements) for i in
          range(self.dim)])
